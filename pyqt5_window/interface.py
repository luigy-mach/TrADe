...
from .gui.view        import class_view
from .gui.controller  import class_controller
from .gui.model       import class_model
...
def main_GUI_eval( beta, eta, tau, query_path, imgs_path, numShowImgs, colNames, dictResult, save_path, title=None):
    ...
    del model
    del view
    # sys.exit(pyEval.exec_()) ## exit todo
...
def main_evaluation(beta, eta, tau, query_id, query_path, imgs_path, gts_csv, reid_csv, save_path, name_file, title=None, show_debug=False):
    ...
    for i in range(1, _numImgs+1):
        _col_names.append('Rank_{:02d}'.format(i))
    values = [0 for _ in range(len(_col_names))]
    
    _dataFrame   = pd.DataFrame(columns=_col_names)
